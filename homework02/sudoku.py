...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    while N > 0:
        sud_i = random.randint(0, 8)
        sud_j = random.randint(0, 8)
        if grid[sud_i][sud_j] != '.':
            grid[sud_i][sud_j] = '.'
            N -= 1
    return grid
